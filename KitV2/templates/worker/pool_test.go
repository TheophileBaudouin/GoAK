...
func TestOptionsWithNilValues(t *testing.T) {
	wp := workerpool.NewWorkerPool(
		workerpool.WithContext(context.TODO()),
...
func TestUnimplementedHandler(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	if err := wp.Submit(ctx, workerpool.Job{ID: "unimplemented", Data: "test"}); err != nil {
		t.Fatalf("unexpected submit error: %v", err)
	}
...
func TestOnErrorCallback(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		ctx, cancel := context.WithTimeout(context.Background(), time.Second)
		if err := wp.Submit(ctx, workerpool.Job{ID: "fail"}); err != nil {
			t.Fatalf("unexpected submit error: %v", err)
		}
		cancel()
	}
...
func TestMetricsHook(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		ctx, cancel := context.WithTimeout(context.Background(), time.Second)
		if err := wp.Submit(ctx, workerpool.Job{ID: "ok"}); err != nil {
			t.Fatalf("unexpected submit error: %v", err)
		}
		cancel()
	}
