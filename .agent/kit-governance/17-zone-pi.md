...
| --- | --- | --- |
| `.pi/prompts/*.md` | **Orchestrators** invoked manually (`/checklist-api`, `/goak`) | workflow-memory, checklist-api, checklist-release, goak |
...
| `.pi/docs/GOAK.md` | **Consumer user guide** — shipped source of truth for using the kit; read by `/goak` and the onboarding banner, works with no other documentation | — |
...
2. **Every prompt has a `description`** of activation. Workflow/checklist orchestrators follow the `workflow-*` / `checklist-*` naming convention; the kit entry-point command `/goak` (`.pi/prompts/goak.md`) is the single deliberate exception — the filename is the command name, and the entry point is a stable contract (D-2026-08-08-15).
