...
The KitV2 gate and probes pass through the 2026-08-05 catalog-hardening wave: 65 product skills, router index 254 resources, strict catalog mode PASS, validator tests PASS, full Go gate PASS, and 5/5 probes PASS. All 42 catalog SKILL.md files were re-read/re-written with fresh primary web research, dated `Sources vérifiées`, coherent single-source sections, and corrected examples; 28 atomic/theme commits plus the governance commit are recorded after f6082f5. Residual: strict mode is documented and tested locally but CI still needs an explicit environment flag if it should become blocking there.