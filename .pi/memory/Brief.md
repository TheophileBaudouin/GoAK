...
**Consumer onboarding & knowledge system (2026-08-08, D-2026-08-08-14..18)**: the kit ships a self-contained onboarding surface — user guide `KitV2/.pi/docs/GOAK.md` (shipped source of truth, works with no metaproject docs), `/goak-help` entry point (`.pi/prompts/goak-help.md`, orders the agent to read the guide), onboarding banner (`.pi/extensions/kit-onboarding.ts` appends `.pi/onboarding/banner.md` once as a chat-transcript entry on startup/reload — not a persistent widget), and a marker-delimited "User guide" section in `KitV2/AGENTS.md`. Enforced by `validate-kitv2.py check_consumer_onboarding` + `kit audit` C18; maintenance obligation encoded in root AGENTS.md + Agent.md + Z8.
...
- **Consumer AGENTS.md writing protocol (D-2026-08-08-19)**: `KitV2/AGENTS.md` is the consumer agent file. Any rewrite MUST follow Z9 §9 (canonical section order, stable MUST/SHOULD/MAY levels, one rule per decision, no history, no duplication, ≤ 16 KiB, three marker sections preserved) and end with the full gate + fresh-context review. Its "UI work" section is a condensed delegation (activation guard + routing + invariants only; every other UI instruction lives in `ui-kit/AGENTS.md`, the single canonical source) with a sha256 marker tripwire refreshed at every ui-kit re-sync (Z13 §4 revised). Enforced by `check_agents_md_contract` (validate-kitv2.py).
