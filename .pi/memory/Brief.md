...
2026-08-06 release v2.5.0: full audit-fix wave KVA-101..110 closed (structure.md mechanism Layer 5.1 — tools/generators/structure_md.py + 3 template reading maps + validator drift gate; enforcement registry complete; no metaproject paths in shipped docs + .agent/ scan; English-only stopwords; usage-evidence recorded; CI gate = local gate with PyYAML pinned, coverage floor green at 81.5% on the library surface, probes excluded by design D-2026-08-06-13). Gate PASS (validators, router 22/22, structure.md 3/3, unit tests, gofmt/vet/lint/race/gosec/govulncheck, probes 15/15). Commits 8757f6d (preexisting 2.5.0 waves), ced6285 (audit fixes), 685e74b (release announcement); tag v2.5.0 pushed; end-to-end install verified from the remote (validator PASS, 15/15 probes, zero .agent/ leakage). Fresh-context review APPROVE after PyYAML + NIT fixes. Plan docs/plans/2026-08-06-audit-fix-wave.md; decisions D-2026-08-06-12..14.

2026-08-07 — macOS Computer Use resources (D-2026-08-07-01..03): premise of an "anti-CGO/Objective-C rule" verified false — no such rule exists; zero-CGO is a library-selection preference only. Added `knowledge/architecture/macos-computer-use.yaml` (Pattern: AXUIElement primary semantic layer, RobotGo execution layer, ScreenCaptureKit visual complement; positive native-API guidance; darwinkit not admitted — stale since 2024-07) + vetted fiche `knowledge/catalogs/libraries/robotgo/SKILL.md` (v1.0.2 stable 2026-03; macOS cgo default, purego via `-tags purego`). Router 280 resources, capabilities 73/44, full gate PASS (validators, 22/22 scenarios, gofmt/vet/lint 0, test -race, gosec 0, govulncheck 0, probes 15/15), fresh review APPROVE-WITH-NITS. Plan docs/plans/2026-08-07-computer-use-macos.md; evidence docs/evidence/2026-08-07/computer-use-macos/.
