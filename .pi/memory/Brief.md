...
2026-08-04 — **Format fiche des catalogues (28 bibliothèques enrichies)** : chaque SKILL.md de `knowledge/catalogs/libraries/` porte désormais 6 sections décisionnelles obligatoires — Utiliser cette librairie quand / Ne pas utiliser cette librairie quand / Avantages / Inconvénients / Pièges connus / Sources vérifiées — ajoutées après les sections vétées (Selection, Admission, Minimal use, Alternatives, Notes), en-têtes FR (spec utilisateur), critiques négatives multi-sources. Placement contraint par la gouvernance : `knowledge/catalogs` est un répertoire de skills Pi déclaré → la fiche vit DANS le SKILL.md (un .md compagnon non-SKILL.md casserait la découverte Pi). Standard encodé : N1 §4 + Z2 §9.2.
## Verification status
