...
**Consumer onboarding & knowledge system (2026-08-08, D-2026-08-08-14..17)**: the kit ships a self-contained onboarding surface — user guide `KitV2/.pi/docs/GOAK.md` (shipped source of truth, works with no metaproject docs), `/goak` entry point (`.pi/prompts/goak.md`, orders the agent to read the guide), onboarding banner (`.pi/extensions/kit-onboarding.ts` renders `.pi/onboarding/banner.md` as a session-start widget on startup/reload), and a marker-delimited "User guide" section in `KitV2/AGENTS.md`. Enforced by `validate-kitv2.py check_consumer_onboarding` + `kit audit` C18; maintenance obligation encoded in root AGENTS.md + Agent.md + Z8.
