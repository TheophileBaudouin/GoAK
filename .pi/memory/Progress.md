...
- [ ] [2026-08-08] Consumer AGENTS.md rewrite protocol (D-2026-08-08-19) : garde-fous méta-projet (Z9 §9 protocole d'écriture, Z13 §4 délégation UI condensée, règle absolue root AGENTS.md + Agent.md, check_agents_md_contract dans validate-kitv2.py (budget 16 KiB, sections canoniques + ordre, interdiction historique) + 6 tests, registre .agent/instructions.md mis à jour) puis réécriture complète de KitV2/AGENTS.md (structure canonique, légende MUST/SHOULD/MAY, routing règle d'entrée, workflow décomposé, historique supprimé, UI déléguée à ui-kit/AGENTS.md, PASS/PARTIAL/FAIL formalisé, écho final des invariants) ; gate complète verte (validators ×3, 35/35 tests, router 23/23 + UI 11/11, Go gate, probes 16/16), revue fresh-context 0 blocker (1 nit corrigé), évidence docs/evidence/2026-08-08/agents-md-rewrite/ + plan docs/plans/2026-08-08-agents-md-rewrite-protocol.md.
- [ ] [2026-08-08] Kit audit KVA-001..005 fix wave : .pi/README.md réécrit (SDK UI = registre unique, inert by description — supprime la contradiction avec settings.json), capabilities.yaml 13→14 rule modules, tools/README.md + ligne sync-ui-kit.sh, ui-kit/.pi vide supprimé + garde-fou helper élargi (échec sur tout résidu .pi), validate-instructions.py PROCESS_SURFACES étendu aux recipes + ligne registre (contrat fonctionnel openapi-validation, frontière KVA-106) ; gate complète verte (validators ×3 + strict, build_index, scenarios 23/23 + UI 11/11, probes 16/16, sync --check OK) ; NON commité (attente utilisateur).
- [ ] [2026-08-08] Consumer AGENTS.md mandatory top blocks (D-2026-08-08-21) : deux blocs propriétaires immuables ajoutés en tête de KitV2/AGENTS.md (« ## Before doing anything » — délégation sub-agents, « ## Absolute rules » — discipline to-do list), règle méta-projet câblée : Z9 §9.1/§9.1a (structure canonique + blocs verbatim), check_agents_md_contract étendu (présence + ordre + phrases sentinelles → gate rouge si blocs perdus/renommés/relocalisés), 3 tests unitaires négatifs ajoutés (38 tests OK), registre .agent/instructions.md 2 lignes (contrôle mécanique nommé, plus « guidance only »), AGENTS.md racine + Agent.md mis à jour ; gate complète verte (validators ×3 + strict, 38 tests, build_index, scenarios 23/23 + UI 11/11, probes 16/16) ; warning budget 12,7 KiB (informatif) ; NON commité (attente utilisateur).
- [ ] [2026-08-08] Release v2.7.0→v2.7.1→v2.7.2 : README racine réécrit (présentation complète du projet, get started, install 1-commande), bump manifest/capabilities/install/GOAK, index router régénéré ; v2.7.0 a shipé un install.sh au défaut GAK_REF périmé (multi-edit drop — gotcha enrichi), corrigé + tag re-pinné, cache raw resté périmé → tag neuf v2.7.1 ; banner onboarding Get Started élargi (/workflow-memory) → v2.7.2 ; installation en 1 commande vérifiée de bout en bout depuis chaque tag (raw + codeload), tree installé sans fuite méta-projet, blocs AGENTS.md présents.
